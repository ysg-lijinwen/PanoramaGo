...
void main() {
    ...
    if(modetype==1){//全景模式
        ...
        if(devicetype ==1 && false){ //这里是有ＢＵＧ的，
            float  xx = v_pos_x;
            float  zz = v_pos_z;
            ...
            gl_FragColor =  getColor(colA,colB,degree,v_maskdegree);;
        }else{
            float  point;
            point = v_pos_x;
        ...
        { 	float point = v_pos_x;
            if((point <= 0.5 - v_masksize )&& (point>=-0.5+v_masksize)){
                alpha = 0.0;
            }else if((point >= 0.5 + v_masksize )|| (point<=-0.5-v_masksize)){
                alpha = 1.0;
            }else if((point>0.5-v_masksize)&&(point<0.5+v_masksize)){
                float  index =  point - (0.5 - v_masksize);
