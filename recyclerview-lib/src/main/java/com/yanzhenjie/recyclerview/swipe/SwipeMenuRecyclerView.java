...
import com.yanzhenjie.recyclerview.swipe.refresh.HeaderAndFooterWrapper;
...
public class SwipeMenuRecyclerView extends RecyclerView {
    ...
    private OnSwipeMenuItemClickListener mDefaultMenuItemClickListener = new OnSwipeMenuItemClickListener() {
        @Override
        public void onItemClick(Closeable closeable, RecyclerView.ViewHolder holder, int menuPosition, @DirectionMode int direction) {
            if (mSwipeMenuItemClickListener != null) {
                mSwipeMenuItemClickListener.onItemClick(closeable, holder, menuPosition, direction);
            }
        }
    };
    ...
    public void setAdapter(Adapter adapter) {
        if (adapter instanceof SwipeMenuAdapter) {
            ...
            HeaderAndFooterWrapper adapter1 = (HeaderAndFooterWrapper) adapter;
            Adapter innerAdapter = adapter1.getInnerAdapter();
            if(innerAdapter instanceof SwipeMenuAdapter){
                SwipeMenuAdapter innerAdapter1 = (SwipeMenuAdapter) innerAdapter;
                innerAdapter1.setSwipeMenuCreator(mDefaultMenuCreator);
                innerAdapter1.setSwipeMenuItemClickListener(mDefaultMenuItemClickListener);
            }
        }
    ...
    public void smoothOpenMenu(int position, @DirectionMode int direction, int duration) {
        ...
        ViewHolder vh = findViewHolderForLayoutPosition(position);
        if (vh != null) {
