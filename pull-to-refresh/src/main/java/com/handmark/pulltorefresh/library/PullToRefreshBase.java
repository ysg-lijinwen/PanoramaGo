...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public void setTotalPage(int totalPage) {
        ...
        Utils.setLoadingLayout(mContext, getLoadingLayoutProxy(false, true));
    }
...
//                }
    ...
    protected final void onRestoreInstanceState(Parcelable state) {
        if (state instanceof Bundle) {
            ...
            Parcelable parcelable = bundle.getParcelable(SUPER_ALL);
            if (parcelable != null) {
    ...
    private void callRefreshListener() {
        if (null != mOnRefreshListener) {
            ...
            if (mCurrentMode == Mode.PULL_FROM_START) {
                mCurPageNo = 1;
                Utils.setLoadingLayout(mContext, getLoadingLayoutProxy(false, true));
                mOnRefreshListener2.onPullDownToRefresh(this, mCurPageNo);
            } else if (mCurrentMode == Mode.PULL_FROM_END) {
                mCurPageNo++;
                if (mCurPageNo > mTotalPage) {
                    Utils.setFinishLayout(mContext, getLoadingLayoutProxy(false, true));
                    ...
                    mOnRefreshListener2.onPullUpToRefresh(this, mCurPageNo);
                }
            }
        }
    }
    ...
    public interface OnRefreshListener2<V extends View> {
         ...
         */
        void onPullDownToRefresh(final PullToRefreshBase<V> refreshView, int curPageNo);

        /**
         * onPullUpToRefresh will be called only when the user has Pulled from
         * the end, and released.
         */
        void onPullUpToRefresh(final PullToRefreshBase<V> refreshView, int curPageNo);
